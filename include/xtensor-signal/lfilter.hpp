    ...
    namespace signal
    {
        namespace detail
        {
            ...
            inline auto lfilter(E1&& b, E2&& a, E3&& x, E4 zi)
            {
                ...
                {
                    XTENSOR_THROW(std::runtime_error, "Accumulator initialization must be the same length as the input");
                }
                if(x.dimension() != 1)
                {
                    XTENSOR_THROW(std::runtime_error, "Implementation only works on 1D arguments");
                }
                if(a.dimension() != 1)
                {
                    XTENSOR_THROW(std::runtime_error, "Implementation only works on 1D arguments");
                ...
                for (int i = 0; i < x.shape(0); i++)
                {
                    ...
                    auto b_accum = xt::sum(b * xt::flip(xt::view(padded_x, xt::range(i, i + b.shape(0))))) + zi(i);
                    auto padded_out = xt::pad(out, a.shape(0) - 1);
                    auto a_accum = b_accum - xt::sum(xt::view(a, xt::range(1, xt::placeholders::_)) * xt::flip(xt::view(padded_out, xt::range(i, i + a.shape(0) - 1))));
                    auto final = a_accum / a(0);
                    out(i) = final();
                }
                return out;
            }
        ...
        /*
        * @brief performs a 1D filter operation along the specified axis. Performs operations immediately.
        ...
        */
        template<typename E1, typename E2, typename E3, typename E4 = decltype(xt::xnone())>
        inline auto lfilter(E1&& b, E2&& a, E3&& x, std::ptrdiff_t axis = -1, E4 zi = xt::xnone())
        {
            using value_type = typename std::decay_t<E3>::value_type;
            ...
            for (auto iter = begin; iter != end; iter++)
            {
                ...
                {
                    xt::noalias(*iter) = detail::lfilter(b, a, *iter, xt::zeros<value_type>({(*iter).shape(0)}));
                }
            }
            return out;
        }
