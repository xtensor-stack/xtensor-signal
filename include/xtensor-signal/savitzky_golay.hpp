...
namespace detail
{
    ...
    public:
            ...
            data_length(_data_length)
        {
            ...
            _toeplitzMatrix = GenerateToeplitzMatrix();
        }

        xt::xarray<float> Transform(xt::xarray<float> data)
        {
        ...
        xt::xarray<double> GenerateKernels()
        {
            const auto rows = 2 * width + 1;
            const auto cols = polyorder + 1;
            ...
            xt::xarray<double> coeffs = xt::zeros<double>({ width + 1,  rows });
            ...
            for (size_t i = 0; i < width + 1; ++i)
            {
                ...
                b1(i) = 1.0;
                xt::view(coeffs, i, xt::all()) = CalculateConvolutionCoefficients(b1, A_template);
            }

            return coeffs;
        }

        xt::xarray<float> GenerateToeplitzMatrix()
        {
            //generate the kernels offline
            auto kernels = GenerateKernels();

            //some cool math here
            xt::xarray<float> toeplitzMatrix = xt::zeros<float>({ data_length, data_length });
            const int64_t window = 2 * static_cast<int64_t>(width) + 1;
...
xt::xarray<double> savgol_filter(xt::xarray<double> x, size_t window_length, size_t polyorder, std::ptrdiff_t axis = -1)
{
    ...
    detail::SavGolTransform filter(x.shape(saxis), window_length, polyorder);
